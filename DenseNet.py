...
# 모델 정의 (DenseNet121 사용)
class PneumoniaModel(nn.Module):
    def __init__(self):
        super(PneumoniaModel, self).__init__()
        self.model = models.densenet121(weights=models.DenseNet121_Weights.DEFAULT)  # 사전 학습된 DenseNet121 모델 사용
        # Dropout을 포함한 추가 레이어로 과적합 방지 및 출력 레이어 변경
        self.model.classifier = nn.Sequential(
            nn.Dropout(0.5),  # 과적합 방지를 위한 Dropout 추가
            nn.Linear(self.model.classifier.in_features, 1)  # 출력 뉴런 수를 1로 설정 (이진 분류)
        )
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0  # patience 카운터 초기화
            torch.save(model.state_dict(), "pneumonia_detection_model_densenet.pth")  # 최적의 모델 저장
            print("모델 저장됨")
        else:
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("pneumonia_detection_model_densenet.pth"))  # 최적의 모델 로드
    model.eval()  # 모델을 평가 모드로 설정
...
# -Kaggle-main\Chest-X-Ray-Kaggle-main'; & 'c:\Python312\python.exe' 'c:\Users\AERO\.vscode\extensions\ms-python.debugpy-2024.12.0-win32-x64\bundled\libs\debugpy\adapter/../..\debugpy\launcher' '50247' '--' 'c:\Users\AERO\Downloads\Chest-X-Ray-Kaggle-main\Chest-X-Ray-Kaggle-main\DenseNet.py' 
# Epoch 1/20, Loss: 0.1785, LR: 0.0009757729755661011
# Validation Loss: 0.1277, Accuracy: 0.6250
# 모델 저장됨
# Epoch 2/20, Loss: 0.1034, LR: 0.000905463412215599
# Validation Loss: 0.2099, Accuracy: 0.6875
# Epoch 3/20, Loss: 0.0878, LR: 0.0007959536998847742
# Validation Loss: 0.1342, Accuracy: 0.6875
# Epoch 4/20, Loss: 0.0805, LR: 0.000657963412215599
# Validation Loss: 0.1691, Accuracy: 1.0000
# Epoch 5/20, Loss: 0.0669, LR: 0.000505
# Validation Loss: 0.1197, Accuracy: 0.6250
# 모델 저장됨
# Epoch 6/20, Loss: 0.0465, LR: 0.0003520365877844011
# Validation Loss: 0.1407, Accuracy: 1.0000
# Epoch 7/20, Loss: 0.0426, LR: 0.00021404630011522585
# Validation Loss: 0.0562, Accuracy: 0.8125
# 모델 저장됨
# Epoch 8/20, Loss: 0.0393, LR: 0.00010453658778440107
# Validation Loss: 0.1623, Accuracy: 1.0000
# Epoch 9/20, Loss: 0.0296, LR: 3.4227024433899005e-05
# Validation Loss: 0.0782, Accuracy: 1.0000
# Epoch 10/20, Loss: 0.0260, LR: 1e-05
# Validation Loss: 0.0931, Accuracy: 1.0000
# Epoch 11/20, Loss: 0.0271, LR: 3.4227024433899005e-05
# Validation Loss: 0.0993, Accuracy: 1.0000
# Epoch 12/20, Loss: 0.0183, LR: 0.00010453658778440146
# Validation Loss: 0.0790, Accuracy: 1.0000
# 조기 종료 발동
# c:\Users\AERO\Downloads\Chest-X-Ray-Kaggle-main\Chest-X-Ray-Kaggle-main\DenseNet.py:154: FutureWarning: You are using `torch.load` with `weights_only=False` (the current default value), which uses the default pickle module implicitly. It is possible to construct malicious pickle data which will execute arbitrary code during unpickling (See https://github.com/pytorch/pytorch/blob/main/SECURITY.md#untrusted-models for more details). In a future release, the default value for `weights_only` will be flipped to `True`. This limits the functions that could be executed during unpickling. Arbitrary objects will no longer be allowed to be loaded via this mode unless they are explicitly allowlisted by the user via `torch.serialization.add_safe_globals`. We recommend you start setting `weights_only=True` for any use case where you don't have full control of the loaded file. Please open an issue on GitHub for any issues related to this experimental feature. 
#   model.load_state_dict(torch.load("pneumonia_detection_model_densenet.pth"))  # 최적의 모델 로드
